...
const FetchTasks = () => {
    ...
    const { setUpdatePayload, setUpdate, setCreateTask,completedTasks, setCompletedTasks, setGroupDataList, setShowList } = useContext(TaskAppContext);
    ...
    // function to group tasks based on category
    const groupTasksByCategory = (tasks = []) => {
        console.log('tasks sent from groupTasksByCate: ',tasks);
        const groupedDataList = tasks.reduce((groups, task) => {
            const category = task.CATEGORY;
            if (!groups[category]) {
                groups[category] = [];
            }
            groups[category].push(task);
            console.log('final groups before groupedList is: ',groups);
            return groups;
            }, {});
            console.log('Data list after grouping is: ',groupedDataList);
            setGroupDataList(groupedDataList);
            setShowList(false);
            return;
    };

    // function to fetch categories for all the open tasks
    const fetchAllOpenCategories = async () => {
        try {
            setHeading('Open');
            ...
            <View style={styles.containedList}>
                {selectTasks ?
                    ...
                    <View>
                        ...
                        {dataList.map((dataValue, index) => (
                            <View style={styles.activeTasksStyle} key={index}>
                                <TouchableOpacity style={styles.activeStyleRecord}>
                                    <Text style={styles.activeStyleRecordTxt}>{dataValue.CATEGORY}</Text>
                                    <Text style={styles.activeStyleRecordTxt}>View/Edit</Text>
                                </TouchableOpacity>
                            </View>
                        ))}
        ...
        <View style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={true} style={styles.listContainer} indicatorStyle="black" borderWidth={dataList && dataList.length>0 ? 5 : 0} >
                {dataList && dataList.length > 0 ? <RenderingCustomList /> :
                    noTasks ? <Text style={styles.taskHeading}>{heading}</Text> : null
                }
            </ScrollView>
            {selectTasks ?
